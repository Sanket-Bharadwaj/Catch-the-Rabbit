  ...
  color: 0x100707, // Retained for rabbit iris
  shading: THREE.FlatShading,
});
var brownMat = new THREE.MeshPhongMaterial({
  color: 0xffffff, // White for rabbit torso, head, ears, mouth
  shininess: 0,
  shading: THREE.FlatShading,
});
var greenMat = new THREE.MeshPhongMaterial({
  color: 0x7abf8e, // Retained for carrot leaves, floor
  shininess: 0,
  shading: THREE.FlatShading,
});
var pinkMat = new THREE.MeshPhongMaterial({
  color: 0xffa500, // Vibrant orange for carrot body
  shininess: 0,
  shading: THREE.FlatShading,
});
var lightBrownMat = new THREE.MeshPhongMaterial({
  color: 0xffb6c1, // Light pink for rabbit nose, tail, paws
  shading: THREE.FlatShading,
});
var whiteMat = new THREE.MeshPhongMaterial({
  color: 0xffffff, // White for rabbit pants, eyes
  shading: THREE.FlatShading,
});
var skinMat = new THREE.MeshPhongMaterial({
  color: 0xffb6c1, // Light pink for rabbit consistency
  shading: THREE.FlatShading
});

    // Game Mode Parameters
    var gameModes = {
      easy: { initSpeed: 3, maxSpeed: 30, monsterAcceleration: 0.002, levelUpdateFreq: 5000 },
      medium: { initSpeed: 5, maxSpeed: 48, monsterAcceleration: 0.004, levelUpdateFreq: 3000 },
      hard: { initSpeed: 7, maxSpeed: 60, monsterAcceleration: 0.006, levelUpdateFreq: 2000 }
    };
    ...
    function applyGameMode(mode) {
      var selectedMode = gameModes[mode];
      initSpeed = selectedMode.initSpeed;
      maxSpeed = selectedMode.maxSpeed;
      monsterAcceleration = selectedMode.monsterAcceleration;
      levelUpdateFreq = selectedMode.levelUpdateFreq;
    }
    ...
    function resetGame() {
      ...
      monsterPos = .56;
      monsterPosTarget = .65;
      var mode = document.getElementById("gameModeSelector").value;
      applyGameMode(mode);
      speed = initSpeed;
      level = 0;
      distance = 0;
    ...
    function initUI() {
      ...
      gameModeSelector.addEventListener("change", function() {
        if (gameStatus == "play" || gameStatus == "readyToReplay") {
          resetGame();
        }
      });
    }
